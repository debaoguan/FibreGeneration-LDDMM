function writevtkfile_standard(fn,varargin)					
...
%					
% This function writes 3D meshes and associated scalar/vector/tensor fields to a VTK (Visualization T	ool Kit)	(www	.k	it	ware.com) dataset file
%					
%					
...
%					
% Depending wether the field is piecewise linear or piecewise constant and if it it scalar, vector or	tensor,	val	c	an	assume different valid sizes:
%					
% val should have a number of rows equal to the number of simplecies or to the number of nodes in the	mesh (p	iece	wi	se	constant/piecewise linear dataset)
...
%					
% CAVEAT: VTK supports only symmetrix tensors. It is left to the user to ensure that the data he is e	xporting				
% consists of symmetric tensors (as if such a check were performed by writeVTKfile, performance of th	e filter	wou	ld	s	uffer)
...
function writevtkfile_old(filepath,vtx,elem,val)					
%					
...
% DA FARE: mettere check sul numero di parametri di ingresso (e sul tipo, es controllare che filename	sia una	str	in	ga	)
...
fprintf('The number of columns in the val vector should be 1,3 or 9, depending if it	reperese	nts	a	sc	alar, a vector field or a tensor field.\n');
...
fprintf('The number of rows of the val vector did not match the number of rows in vtx or in e	lem.\n')	;			
fprintf('A VTK file containing just mesh information was written.\n');					
